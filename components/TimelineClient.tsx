          ...
          {Object.entries(groupedApps).map(([monthKey, monthApps]) => (
            <div key={monthKey}>
              ...
              <div className="mt-4 relative space-y-6">
                <div className="absolute top-0 bottom-0 left-16.5 w-0.5 bg-zinc-200 dark:bg-zinc-800 hidden sm:block rounded" />
                {monthApps.map((app) => {
                  const stageCfg =
                  ...
                  return (
                    ...
                    >
                      <div className="sm:w-12 pt-1.5 shrink-0 sm:text-right hidden sm:block">
                        ...
                        <span className="text-sm font-extrabold text-zinc-900 dark:text-zinc-100 leading-none">
                          {dateStr.split(" ")[1]}
                        </span>
                      </div>
                      <div className="relative z-10 sm:mt-1.5 hidden sm:flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white dark:bg-zinc-900 ring-4 ring-zinc-50 dark:ring-zinc-950 border border-zinc-200 dark:border-zinc-700">
                        <div
                          className={`w-2.5 h-2.5 rounded-full ${stageCfg.dot}`}
                        />
                      </div>
