...
const COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  offer: { bg: "bg-teal-50", text: "text-teal-700", dot: "bg-teal-500" },
...
export default function TimelineClient({ applications }: { applications: ApplicationRow[] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      {/* Filters */}
      <div className="flex flex-wrap gap-2 mb-8 border-b border-[#DFE1E6] pb-6">
      ...
      {filteredApps.length === 0 ? (
        <div className="bg-white border border-[#DFE1E6] rounded-xl p-12 text-center text-[#5E6C84]">
          ...
          <p className="font-semibold text-[#172B4D]">No applications found</p>
          <p className="text-sm mt-1">Try selecting a different status filter.</p>
        </div>
      ) : (
        <div className="space-y-10">
          {Object.entries(groupedApps).map(([monthKey, monthApps]) => (
            <div key={monthKey}>
              {/* Month Header */}
              <div className="sticky top-0 z-10 py-3 bg-[#F4F5F7]">
                <h3 className="text-sm font-bold text-[#172B4D] tracking-wide">
                  {monthKey}
                </h3>
              </div>
              ...
              <div className="mt-4 relative space-y-6">
                ...
                <div className="absolute top-0 bottom-0 left-[66px] w-[2px] bg-[#DFE1E6] hidden sm:block rounded" />
                  ...
                  return (
                    <div key={app.id} className="relative flex flex-col sm:flex-row gap-4 sm:gap-6 items-start group">
                      ...
                      <div className="sm:w-[48px] pt-1.5 shrink-0 sm:text-right hidden sm:block">
                        <span className="text-xs font-bold text-[#5E6C84] block">{dateStr.split(' ')[0]}</span>
                        <span className="text-sm font-extrabold text-[#172B4D] leading-none">{dateStr.split(' ')[1]}</span>
                      </div>

                      {/* Timeline Node overlaying the line */}
                      <div className="relative z-10 sm:mt-1.5 hidden sm:flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white ring-4 ring-[#F4F5F7] border border-[#DFE1E6]">
                      ...
                      <div className="flex-1 w-full relative">
                         ...
                         <div className="flex items-center gap-2 mb-2 sm:hidden pl-1">
                            ...
                            <span className="text-xs font-bold text-[#172B4D]">{dateStr}</span>
                         </div>

                         <div className="bg-white p-4 sm:p-5 rounded-xl border border-[#DFE1E6] shadow-sm hover:shadow-md transition-shadow">
                           <div className="flex justify-between items-start mb-2 gap-4">
                              <div className="flex-1">
                                <h4 className="font-bold text-[#172B4D] text-base leading-tight">
                                  {app.position}
                                </h4>
                                <div className="text-sm font-medium text-[#5E6C84] mt-0.5">
                                  {app.companyName}
                                </div>
                              </div>
                              <span className={`shrink-0 inline-flex items-center px-2.5 py-1 rounded text-[11px] font-bold uppercase tracking-wider ${sc.bg} ${sc.text}`}>
                                {app.status}
                              </span>
                           </div>
                           ...
                           <div className="flex flex-wrap gap-2 mt-4">
                              {app.source && (
                                <span className="inline-flex items-center gap-1.5 bg-[#F4F5F7] px-2 py-1 rounded text-xs font-semibold text-[#5E6C84] border border-[#DFE1E6]">
                              ...
                              {sal && (
                                <span className="inline-flex items-center gap-1.5 bg-green-50 px-2 py-1 rounded text-xs font-semibold text-green-700 border border-green-200">
                                  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5">
                           ...
                           {/* Status Note Footer */}
                           <div className="mt-4 pt-3 border-t border-[#DFE1E6] flex items-center gap-2 text-xs font-medium text-[#5E6C84]">
                             <svg viewBox="0 0 16 16" fill="currentColor" stroke="none" className="w-3.5 h-3.5 text-[#B3BAC5]">
