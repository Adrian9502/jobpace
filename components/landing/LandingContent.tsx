...
import PasswordRequirements, { RequirementItem } from "@/components/ui/PasswordRequirements";
...
export default function LandingContent() {
  ...
  const showRequirements = activeTab === "signup" && (passwordFocused || password.length > 0);

  useEffect(() => {
    if (searchParams.get("verified") === "true") toast.success("Email verified! You can now sign in.");
    if (searchParams.get("reset") === "true") toast.success("Password reset successfully! Sign in with your new password.");
    const error = searchParams.get("error");
    if (error === "invalid_token" || error === "missing_token") toast.error("Invalid verification link. Please request a new one.");
    if (error === "token_expired") toast.error("Verification link has expired. Please request a new one.");
  }, [searchParams]);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result?.error) { setIsGoogleLoading(false); return; }
  ...
  const handleCredentialsSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (result.error && result.error !== "EMAIL_NOT_VERIFIED") setFormError(result.error);
    } catch {
  ...
  const handleResendVerification = async () => {
   ...
   return (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] flex items-center justify-center px-4 sm:px-6 py-10 sm:py-12 relative">
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-2 w-full max-w-5xl rounded-2xl border border-gray-200 dark:border-zinc-800 overflow-hidden shadow-sm transition-all duration-300 ${activeTab === "signup" ? "lg:h-[700px]" : "lg:h-[640px]"}`}>
        ...
        >
          <div className="max-w-110 mx-auto lg:mx-0">
            <div className="flex items-center gap-2 mb-10 sm:mb-12">
              <Image src="/jobpace-logo-blue.png" alt="JobPace" width={33} height={33} className="rounded-lg shadow-sm pointer-events-none" priority />
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold leading-[1.1] tracking-tight text-gray-900 dark:text-white mb-5 sm:mb-6">
              Your job search, <br /><span className="text-blue-600"> at your pace.</span>
            </h1>
            <div className="flex flex-col gap-3 sm:gap-4 text-sm sm:text-[15px] text-gray-500 dark:text-zinc-400 leading-relaxed mb-7 sm:mb-10">
              <p>Applications get lost, follow-ups are forgotten, and the whole process starts to feel like shouting into the void.</p>
              <p>JobPace gives you one place to track everything, so instead of managing chaos, you&apos;re just moving forward.</p>
            </div>
            {/* features */}
    ...
    <div key={f.label} className="flex items-start gap-4">
      ...
      <div className="flex flex-col gap-0.5">
        <span className="text-sm sm:text-[15px] font-bold text-gray-900 dark:text-white">{f.label}</span>
        <span className="text-[13px] sm:text-[14px] text-gray-400 dark:text-zinc-500 leading-snug">{f.desc}</span>
      </div>
    </div>
  ))}
          ...
          <div className="w-full max-w-90">
            <div className="mb-5 text-center">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 dark:text-white mb-2">
                {activeTab === "signin" ? "Sign in to" : "Sign up for"} <span className="text-blue-600">JobPace</span>
              </h2>
              <p className="text-sm text-gray-500 dark:text-zinc-500">
                {activeTab === "signin" ? "Track every application, never miss a follow-up." : "Create your account and start tracking today."}
              </p>
            </div>
              ...
              {showVerificationNotice && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-3 overflow-hidden">
                  <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3.5">
                    <p className="text-sm text-amber-800 dark:text-amber-200 font-medium mb-1.5">Please verify your email address before signing in.</p>
                    <p className="text-xs text-amber-600 dark:text-amber-400 mb-2.5">Check your inbox for a verification link. Didn&apos;t get it?</p>
                    <button onClick={handleResendVerification} disabled={isResending} className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400 cursor-pointer disabled:opacity-50">
                  ...
                  {activeTab === "signup" && (
                    <div className="flex gap-3">
                      <div className="flex-1">
                        <label htmlFor="firstName" className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">First Name</label>
                        <input id="firstName" name="firstName" type="text" required autoComplete="given-name" placeholder="Juan"
                      ...
                      <div className="flex-1">
                        <label htmlFor="lastName" className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Last Name</label>
                        <input id="lastName" name="lastName" type="text" required autoComplete="family-name" placeholder="Dela Cruz"
                  ...
                  <div>
                    ...
                    {activeTab === "signin" && (
                      <div className="mt-1.5 text-right">
                        <Link href="/auth/forgot-password" className="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium">Forgot password?</Link>
                      </div>
                    )}
                  </div>
            ...
            <p className="text-[12px] text-gray-400 text-center leading-relaxed">
              ...
              <button onClick={() => setModalType("terms")} className="text-blue-600 hover:underline cursor-pointer font-medium">Terms</button>
              {" "}and{" "}
              <button onClick={() => setModalType("privacy")} className="text-blue-600 hover:underline cursor-pointer font-medium">Privacy</button>.
            </p>
          </div>
        </motion.div>
      </div>

      <PolicyModal isOpen={modalType === "terms"} onClose={() => setModalType(null)} title="Terms of Service"
        content={<div className="space-y-4"><h3>1. Introduction</h3><p>Welcome to JobPace. By using our service, you agree to these terms.</p><h3>2. Your Account</h3><p>Keep your password confidential. You are responsible for activity on your account.</p></div>} />
      <PolicyModal isOpen={modalType === "privacy"} onClose={() => setModalType(null)} title="Privacy Policy"
        content={<div className="space-y-4"><h3>1. Information We Collect</h3><p>Your name, email, and job application data to provide and improve our service.</p><h3>2. Information Security</h3><p>We protect your data from unauthorized access and disclosure.</p></div>} />
    </div>
 <VerificationEmail verificationUrl="http://localhost:3000" />
    </>
  );
}
...
function FieldError({ errors }: { errors?: string[] }) {
  ...
  return (
    <div className="mt-1.5">
      {errors.map((error, i) => <p key={i} className="text-xs text-red-500 dark:text-red-400">{error}</p>)}
    </div>
  );
}