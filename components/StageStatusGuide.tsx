...
export default function StageStatusGuide() {
  ...
  const guideContent = (
    <div className="space-y-5">
      {/* Stages */}
      <div>
        <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wide mb-2 flex items-center gap-1.5">
          ...
          Stages
          <span className="font-normal normal-case text-zinc-500 dark:text-zinc-400">— where you are in the process</span>
        </h4>
          ...
          {STAGES.map((s) => {
            ...
            return (
              <div key={s} className="flex items-start gap-2 py-1">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded border text-[10px] font-bold uppercase tracking-wider ${cfg.bg} ${cfg.text} ${cfg.border} shrink-0 mt-0.5`} style={{ minWidth: '70px', justifyContent: 'center' }}>
                  {cfg.label}
                </span>
                <div className="min-w-0">
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 ml-1.5">{cfg.description}</span>
                </div>
              </div>
            );
          })}
      ...
      <div>
        <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wide mb-2 flex items-center gap-1.5">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5 text-amber-500">
            <circle cx="8" cy="8" r="6" />
            <path d="M8 5v3l2 2" />
          </svg>
          Statuses
          <span className="font-normal normal-case text-zinc-500 dark:text-zinc-400">{`— what's happening in that stage`}</span>
        </h4>
        <div className="space-y-1">
          {STATUSES.map((s) => {
            ...
            return (
              <div key={s} className="flex items-start gap-2 py-1">
                ...
                <div className="min-w-0">
                  <span className="text-xs font-semibold text-zinc-900 dark:text-zinc-100">{cfg.label}</span>
                  <span className="text-xs text-zinc-500 dark:text-zinc-400 ml-1.5">— {cfg.description}</span>
                </div>
              </div>
            );
      ...
      <div className="bg-blue-50/50 dark:bg-blue-900/10 rounded-lg p-3 border border-blue-100 dark:border-blue-800/30">
        <p className="text-xs text-zinc-700 dark:text-zinc-300">
          <strong>Example:</strong> Stage = <span className="font-semibold text-indigo-600 dark:text-indigo-400">INTERVIEW</span>,
          Status = <span className="font-semibold text-amber-600 dark:text-amber-400">Pending</span> means you&apos;re
          waiting for your interview schedule.
        </p>
      </div>
    </div>
  );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
              <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Stages & Statuses Guide</h3>
              <button
                onClick={() => setOpen(false)}
