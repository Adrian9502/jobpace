...
import { CalendarDays, Clock, AlertTriangle, CheckCircle, Mail, FileText, ExternalLink } from "lucide-react";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {upcoming.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center shadow-sm">
              ...
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">You're all caught up!</h3>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 max-w-md mx-auto">
                No upcoming interviews, due follow-ups, or stale applications right now. Keep applying or take a break!
              </p>
            </div>
          ) : (
            upcoming.map((item) => (
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
          {history.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">No history yet</h3>
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 max-w-md mx-auto">
                When the system sends you automated reminders, they will appear here.
              </p>
            </div>
          ) : (
            <ul className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
              {history.map((item) => (
                <li key={item.id} className="p-4 sm:p-5 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-500 sm:self-start">
                      ...
                      {item.sentAt ? formatDateTime(item.sentAt) : "Unknown date"}
                    </div>
                  </div>
                </li>
              ))}
...
function HistoryIcon({ type }: { type: string }) {
  if (type === "interview_reminder") {
    return <div className="p-1.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-md"><CalendarDays className="w-4 h-4" /></div>;
  }
  if (type === "stale_application_reminder") {
    return <div className="p-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-md"><AlertTriangle className="w-4 h-4" /></div>;
  }
  return <div className="p-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-md"><Mail className="w-4 h-4" /></div>;
}

function getHistoryTitle(type: string) {
  switch (type) {
    case "interview_reminder": return "Interview Reminder Sent";
    case "follow_up_reminder": return "Follow-up Reminder Sent";
    case "stale_application_reminder": return "Stale Application Alert Sent";
    default: return "Notification Sent";
  }
}
