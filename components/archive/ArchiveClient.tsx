...
export default function ArchiveClient({ applications }: Props) {
  ...
  const archivedApps = useMemo(() => {
    return applications.filter((app) => FINAL_STAGE_SET.has(app.stage));
  }, [applications]);
  ...
  const filtered = useMemo(() => {
    return archivedApps.filter((app) => {
      ...
      const matchesOutcome =
        outcomeFilter === "all" || app.stage === outcomeFilter;
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
            {archivedApps.length} completed application
            {archivedApps.length !== 1 ? "s" : ""}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search archive by company, position, or location..."
            value={search}
            onChange={(e) => {
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center shadow-sm">
          <div className="w-16 h-16 bg-zinc-100 dark:bg-zinc-800 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Trash2 className="w-8 h-8 text-zinc-400 dark:text-zinc-500" />
          </div>
          <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-1">
            Archive is empty
          </h3>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4 max-w-sm mx-auto">
            Applications will automatically appear here once they reach a final
            outcome.
          </p>
        </div>
      ) : (
        // ✅ removed overflow-hidden here — that's what was pushing the dropdown far away
            ...
            {paginated.map((app) => (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-end gap-2">
                  <button
                    onClick={() => openView(app)}
                    className="px-3 py-1.5 text-xs font-medium text-zinc-600 dark:text-zinc-300 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md shadow-sm hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors flex-1 text-center flex items-center justify-center gap-1.5"
                  >
                  ...
                  <button
                    onClick={() => setDeleteTarget(app)}
                    className="px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md shadow-sm hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors flex items-center justify-center gap-1.5"
                  >
